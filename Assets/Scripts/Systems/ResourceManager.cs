...
namespace Gamepackage
{
    public class ResourceManager
    {
        ...
        private void LoadBehaviourPrototypes(IDbConnection dbConnection)
        {
            ...
            while (reader.Read())
            {
                BehaviourPrototype prototype = new BehaviourPrototype()
                {
                    UniqueIdentifier = reader.GetString(1),
                    ClassName = reader.GetString(2),
                    BlocksPathing = reader.GetBoolean(3)
                };
                CacheResource(prototype);
            }
            reader.Close();
        ...
        private void LoadItemTables(IDbConnection dbconn)
        {
            IDbCommand dbcmd = dbconn.CreateCommand();
            IDataReader reader;
        ...
        public TPrototype GetPrototypeByUniqueIdentifier<TPrototype>(UniqueIdentifier uniqueIdentifier) where TPrototype : IResource
        {
            try
            {
