    ...
    public class OverlaySystem
    {
        ...
        private void Draw(Overlay overlay)
        {
            ...
            foreach (var config in overlay.Configs)
            {
                ...
                {
                    if (config.ConstrainToLevel && !Context.Game.CurrentLevel.BoundingBox.Contains(point))
                    {
                        continue;
                    }
                    // This duplication of the contains check is necessary 
                    if (config.WalkableTilesOnly && Context.Game.CurrentLevel.BoundingBox.Contains(point) && Context.Game.CurrentLevel.Grid[point.X, point.Y].TileType != TileType.Floor)
                    {
                        continue;
                    }
