        ...
        private void Draw(Overlay overlay)
        {
            ...
            foreach (var config in overlay.Configs)
            {
                ...
                foreach (var point in points)
                {
                    ...
                    if (config.WalkableTilesOnly && Context.GameStateManager.Game.CurrentLevel.BoundingBox.Contains(point) && Context.GameStateManager.Game.CurrentLevel.Grid[point.X, point.Y].TileType != TileType.Floor)
                    {
                        continue;
                    }
