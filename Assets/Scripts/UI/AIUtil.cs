...
namespace Gamepackage
{
    ...
    {
        [JsonIgnore]
        private static readonly List<AttackType> MeleeAIRelevantContexts = new List<AttackType>()
        {
            AttackType.Melee,
        };

        [JsonIgnore]
        private static readonly List<AttackType> RangedAIRelevantContexts = new List<AttackType>()
        {
            AttackType.Melee,
            AttackType.Zapped,
            AttackType.Thrown,
        ...
        {
            entity.Behaviour.PathsReturned++;
            if (path.Nodes.Count > 0)
            {
                ...
                entity.Behaviour.NextAction = move;
            }
        }

        public static List<AttackType> RangedAIRelevantContexts1 => RangedAIRelevantContexts;

        public static void CommonAIAttackWithWeapon(Level level, Entity entity, Item item, List<AttackType> contexts)
        {
            ...
            {
                entity.Behaviour.LastKnownTargetPosition = new Point(hostileTarget.Position);
        ...
        private static bool AbleAndWillingToPerformAttackTypeOnTarget(Entity source, AttackType attackType, Item item, Entity target)
        {
        ...
        {
            entity.Behaviour.PathsReturned = 0;
            if (entity.Behaviour.AI == AIType.None)
            {
        ...
        private static void EnqueueAttackType(Entity entity, AttackType attackType, Item item, Entity target)
        {
            var grid = Context.Game.CurrentLevel.Grid;
            ...
            entity.Behaviour.NextAction = attack;
        }

        public static void EnqueueBasicAttackOrApproach(Entity entity, Item item, List<AttackType> relevantContexts, Entity target)
        {
            ...
            if (entity.Behaviour.NextAction == null) // was never able to resolve a thing to do
            {
        ...
        {
            MakeNoiseIndicatingLocation(level, source.Position, hostileTarget.Position, shoutRadius, (possibleEntity) => { return possibleEntity.Behaviour != null && possibleEntity.Behaviour.ActingTeam == source.Behaviour.ActingTeam; });
        }
        ...
        public static IEnumerator MoveToward(Entity entity, Point targetPosition)
        {
            ...
            entity.Behaviour.PathsExpected = PointsAroundTarget.Count + PointsAroundMe.Count;
            ...
            while (entity.Behaviour.NextAction == null)
            {
                if (entity.Behaviour.PathsReturned == entity.Behaviour.PathsExpected)
                {
                    var wait = new Wait
                    {
                        Source = entity
        ...
        public static void EnqueueDefaultBehaviour(Entity entity)
        {
            ...
            {
                if (entity.Behaviour.LastKnownTargetPosition != null)
                {
                    if (entity.Behaviour.LastKnownTargetPosition.Distance(entity.Position) < 1.0f)
                    {
                        entity.Behaviour.LastKnownTargetPosition = null;
                        var wait = new Wait
                        {
                            Source = entity
                        };
                    ...
                    {
                        Context.GameSceneRoot.StartCoroutine(MoveToward(entity, entity.Behaviour.LastKnownTargetPosition));
                        return;
                    }
                }
                else
